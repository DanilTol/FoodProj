...
namespace FoodService.Business.Services
{
    public class OrderService : IOrderService
    {
        ...
        {
            var fromDb = Database.OrderDishes.QueryToTable.Where(x => x.Order.Date == date.Date && x.Order.User.id == user.id);
            var dishesInfo = UniteDishAndImage.GetDishImagesFromDbAndUnite(Database, fromDb.Select(x => x.Dish)).ToList();
        ...
        public void UpdateOrder(DateTime date, int[] dishIds,int[] dishNum, User user)
        {
            ...
            if (order == null && dishIds.Length>0)
            {
                ...
                {
                    var temp = new OrderDish() { Dish = Database.Dish.QueryToTable.FirstOrDefault(x => x.id == idAndCount.dishId), Order = newOrder, Count = idAndCount.dishCount };
            ...
            else
            {
                ...
                {
                    var dishIdAndCounts = dishIds.Zip(dishNum, (id, count) => new { dishId = id, dishCount = count });
                    var addOrderDishes = dishIdAndCounts.Select(dishinfo => new KeyValuePair<int, int>(dishinfo.dishId, dishinfo.dishCount)).ToList();

                    //getting all dishes from order(dishes potential for delete)
                    var deleteOrderDishes = order.OrderDishes.Select(x => x.Dish).ToList();

                    //fill equal list
                    var equalDishesInOrderDish = (from newDish in addOrderDishes from oldDish in deleteOrderDishes where oldDish.id == newDish.Key select newDish).ToList();
                    ...
                    {
                        Database.OrderDishes.Delete(Database.OrderDishes.QueryToTable.FirstOrDefault(x => x.Dish.id == old.id && x.Order.id == order.id));
                    }
                    ...
                    {
                        Database.OrderDishes.Add(new OrderDish {Count = orderDish.Value, Order = order, Dish = Database.Dish.QueryToTable.FirstOrDefault(x => x.id == orderDish.Key)});
                    }
                    order.Checked = false;
        ...
        public IEnumerable<OrderInfo> GetOrderListOnWeek(DateTime date)
        {
            ...
            foreach (var order in ordersDb)
            {
                ...
                var dishShort = Mapper.Map<IEnumerable<Dish>,List<DishModelShortInfo>>(order.OrderDishes.Select(dish => dish.Dish));
        ...
        public void SentMailToChef(DateTime date, string chefMail)
        {
            ...
            string messageBody = "<table style='border: 1px solid black'><tr><th>Order Id</th><th>Date</th><th>User</th><th>Role</th><th>Dishes</th></tr>";
